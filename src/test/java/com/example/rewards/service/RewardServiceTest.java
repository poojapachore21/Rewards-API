...
public class RewardServiceTest {
    ...
    public void testCalculateRewards_singlePurchaseAbove100() throws Exception {
        Customer c = new Customer("Test User", "t@example.com");
        // set id via reflection-ish simple approach (not persisted)
        java.lang.reflect.Field f = c.getClass().getDeclaredField("id"); f.setAccessible(true); f.set(c, 1L);

        when(customerRepository.findById(1L)).thenReturn(Optional.of(c));

        Purchase p = new Purchase(new BigDecimal("120.00"), LocalDateTime.of(2025,1,15,10,15,30));
        java.lang.reflect.Field pf = p.getClass().getDeclaredField("id"); pf.setAccessible(true); pf.set(p, 10L);
        p.setCustomer(c);
        ...
        assertEquals(90L, response.getTotalPoints());
    }
